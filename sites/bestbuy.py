...
class BestBuy:
    def __init__(self,task_id,status_signal,image_signal,product,profile,proxy,monitor_delay,error_delay):
        ...
        while True:
            ...
            if not success and jwt != None:
                ...
                if success:
                    self.status_signal.emit({"msg":"Purchased","status":"normal"})
        ...
        while True:
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    self.status_signal.emit({"msg": "(page output: {})".format(r.text), "status": "normal"})
                    if not image_found:
                        if r.text.__contains__("sku product-data"):
                        ...
                        elif r.text.__contains__("list-item lv"):
                            self.sku_id = doc.xpath('//div[@class="list-item lv"]/@data-sku-id')[0]
                            product_image = doc.xpath('//img[@class="product-image"]/@src')[0]
                        self.status_signal.emit({"msg": "(sku id out: {})".format(self.sku_id),"status":"normal"})
    ...
    def atc(self):
        sensor_data = open('data/sensor_data_cookie.txt', 'r').read().replace('\n', '')
        headers ={
        ...
        while True:
            ...
            try:
                ...
                print(r.status_code)
                print(json.loads(r.text)["cartCount"])
    ...
    def select_fufillment(self):
                ...
                body = {"phoneNumber": profile["billing_phone"],"smsNotifyNumber":"","smsOptIn":"false","emailAddress": profile["shipping_email"]}
                while True:
                    self.status_signal.emit({"msg":"Submitting Email","status":"normal"})
                    try:
                        r = self.session.patch("https://www.bestbuy.com/checkout/c/orders/{}/".format(self.order_id),json=body,headers=headers)
                        print(json.loads(r.text))
                        if json.loads(r.text)["id"] == self.order_id:
                            self.payment_id = json.loads(r.text)["payment"]["id"]
                            print(self.payment_id)
                            self.status_signal.emit({"msg":"Submitted Email","status":"normal"})
                            return
                        self.status_signal.emit({"msg":"Error Submitting Email","status":"error"})
                        time.sleep(self.error_delay)
                    except Exception as e:
                        self.status_signal.emit({"msg":"Error Submitting Email (line {} {} {})".format(sys.exc_info()[-1].tb_lineno, type(e).__name__, e),"status":"error"})
                        time.sleep(self.error_delay)    
    ...
    def refresh_payment(self):
                ...
                while True:
                    ...
                    try:
                        r = self.session.post("https://www.bestbuy.com/checkout/d/orders/{}/paymentMethods/refreshPayment".format(self.order_id),json=body,headers=headers)
                        print(json.loads(r.text))
                        if json.loads(r.text)["id"] == self.order_id:
                            self.status_signal.emit({"msg":"Submitted Payment Refresh","status":"normal"})
                            return
                        self.status_signal.emit({"msg":"Error Submitting Payment Refresh","status":"error"})
                        time.sleep(self.error_delay)
                    except Exception as e:
                        self.status_signal.emit({"msg":"Error Submitting Payment Refresh(line {} {} {})".format(sys.exc_info()[-1].tb_lineno, type(e).__name__, e),"status":"error"})
                        time.sleep(self.error_delay)    
    ...
    def submit_shipping(self):
        headers={
            ...
            "content-type": "application/json",
            "origin": "https://www.bestbuy.com",
            "referer": "https://www.bestbuy.com/checkout/c/r/fulfillment",
            "user-agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.92 Safari/537.36",
            "x-user-interface": "DotCom-Optimized"
        }
        profile = self.profile
        ...
        while True:
            ...
            try:
                r = self.session.patch("https://www.bestbuy.com/checkout/c/orders/{}/".format(self.order_id),json=body,headers=headers)
                self.customer_id = json.loads(r.text)["customerOrderId"]
                if json.loads(r.text)["id"] == self.order_id:
    ...
    def submit_payment(self,tas_data):
        ...
        while True:
            ...
            try:
                print("https://www.bestbuy.com/payment/api/v1/payment/{}/creditCard".format(self.payment_id))
                r = self.session.put("https://www.bestbuy.com/payment/api/v1/payment/{}/creditCard".format(self.payment_id),json=body,headers=headers)
                r = json.loads(r.text)
                print(r)
    ...
    def submit_order(self):
        headers = {
            ...
            "content-type": "application/json",
            "origin": "https://www.bestbuy.com",
            "referer": "https://www.bestbuy.com/checkout/d/payment",
            "user-agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.92 Safari/537.36",
            "x-user-interface": "DotCom-Optimized"
        }
        ...
        while True:
            ...
            try:
                r = self.session.post("https://www.bestbuy.com/checkout/d/orders/{}/".format(self.order_id),json=body,headers=headers)
                r = json.loads(r.text)
                print(r)
                try:
                    r = r["errors"][0]
    ...
    def handle_3dsecure(self,jwt):
        ...
        while True:
            ...
            try:
                r = self.session.post("https://centinelapi.cardinalcommerce.com/V2/Cruise/TermRedirection",data=body,headers=headers)
                doc = lxml.html.fromstring(r.text)
